...
def main():
    try:
        ...
        if not args.date:
            rumor_date = fda.fetch_latest_create_date_of_rumor()
            if rumor_date:
        ...
        rumor_infos = fda.parse_rumor_pages(latest_create_date)

        logger.info("FDA Crawler date: {}".format(latest_create_date))
        logger.info("FDA Crawler rumor_infos: {}".format(rumor_infos))

        for rumor_info in rumor_infos:
            (url, date) = rumor_info
            fetched = False
            for rumor in RumorModel.source_create_date_index.query(fda.source,
                                                                   RumorModel.create_date == date,
                                                                   RumorModel.link == url):
                fetched = True

            if not fetched:
                posted_item = fda.parse_rumor_content(rumor_info)
