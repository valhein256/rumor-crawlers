...
def main():
    try:
        ...
        logger.info("TFC Crawler date: {}".format(latest_create_date))
        logger.info("TFC Crawler rumor_infos number: {}".format(len(rumor_infos)))

        for rumor_info in rumor_infos:
            (url, date, _) = rumor_info
            fetched = False
            for rumor in RumorModel.source_create_date_index.query(tfc.source,
                                                                   RumorModel.create_date == date,
                                                                   RumorModel.link == url):
                fetched = True

            if not fetched:
                posted_item = tfc.parse_rumor_content(rumor_info)
